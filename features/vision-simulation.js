export function simulateVision(node) {
    ...
    types.forEach((type, i) => {
        ...
        adjustColorsForColorBlindnessType(clone, COLOR_BLINDNESS_MATRICES[type.key]);
        frames.push(frame);
    });
...
function adjustColorsForColorBlindnessType(node, matrix) {
    if ('fills' in node && Array.isArray(node.fills)) {
        ...
        const newFills = fills.map(fill => {
            if (fill.type === 'SOLID' && 'color' in fill) {
                ...
                if (!(r === 1 && g === 1 && b === 1)) {
                    return Object.assign(Object.assign({}, fill), { color: applyColorMatrix(fill.color, matrix) });
                }
            }
            return fill;
        });
